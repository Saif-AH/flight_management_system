...
export function AirportForm({ airport, serverErrors, isSubmitting, onSubmit, onCancel }: { airport?: Airport; serverErrors: Record<string, string[]>; isSubmitting: boolean; onSubmit: (values: AirportFormValues) => void; onCancel: () => void }) {
  const form = useForm<AirportFormValues>({
    ...
    defaultValues: airport ? { code: airport.code, name: airport.name, city: airport.city, country: airport.country } : { code: '', name: '', city: '', country: '' }
  });
  ...
  return (
    <form className="grid gap-4 sm:grid-cols-2" onSubmit={form.handleSubmit(onSubmit)}>
      ...
      <div className="space-y-2"><Label htmlFor="name">Name</Label><Input id="name" disabled={isSubmitting} {...form.register('name')} /><FieldErrorText error={form.formState.errors.name ?? fieldServerError('name')} /></div>
