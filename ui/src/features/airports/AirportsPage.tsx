...
export function AirportsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Airport directory</CardTitle></CardHeader><CardContent><div className="mb-4 flex gap-3"><Input placeholder="Search by code, city, country..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} /></div>{airports.isLoading ? <TableSkeleton columns={6} /> : airports.data?.items.length ? <><Table><TableHeader><TableRow><TableHead>Code</TableHead><TableHead>Name</TableHead><TableHead>City</TableHead><TableHead>Country</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{airports.data.items.map((airport) => <TableRow key={airport.id}><TableCell className="font-medium">{airport.code}</TableCell><TableCell>{airport.name}</TableCell><TableCell>{airport.city}</TableCell><TableCell>{airport.country}</TableCell><TableCell className="text-right"><Button variant="ghost" size="icon" onClick={() => openEdit(airport)}><Edit className="h-4 w-4" /></Button><Button variant="ghost" size="icon" onClick={() => setDeleteTarget(airport)}><Trash2 className="h-4 w-4 text-destructive" /></Button></TableCell></TableRow>)}</TableBody></Table><Pagination page={page} totalPages={airports.data.totalPages} onPageChange={setPage} /></> : <EmptyState title="No airports found" description="Create an airport or change your search." />}</CardContent></Card>
