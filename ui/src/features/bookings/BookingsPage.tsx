...
export function BookingsPage() {
  ...
  const params = { page, pageSize: 10, status, email, sortBy: 'bookedAtUtc', sortDirection: 'desc' as const };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>Reservations</CardTitle></CardHeader><CardContent><div className="mb-4 grid gap-3 md:grid-cols-3"><Input placeholder="Filter by passenger email" value={email} onChange={(e) => { setEmail(e.target.value); setPage(1); }} /><Select value={status} onChange={(e) => { setStatus(e.target.value); setPage(1); }}><option value="">All statuses</option><option value="Confirmed">Confirmed</option><option value="Cancelled">Cancelled</option></Select></div>{bookings.isLoading ? <TableSkeleton columns={8} /> : bookings.data?.items.length ? <><Table><TableHeader><TableRow><TableHead>Reservation</TableHead><TableHead>Passenger</TableHead><TableHead>Email</TableHead><TableHead>Flight</TableHead><TableHead>Seats</TableHead><TableHead>Booked</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{bookings.data.items.map((booking) => <TableRow key={booking.id}><TableCell className="font-medium">{booking.reservationCode}</TableCell><TableCell>{booking.passengerName}</TableCell><TableCell>{booking.passengerEmail}</TableCell><TableCell>{flightLabel(booking)}</TableCell><TableCell>{booking.seats}</TableCell><TableCell>{formatDateTime(booking.bookedAtUtc)}</TableCell><TableCell><Badge variant={booking.status === 'Cancelled' ? 'destructive' : 'secondary'}>{booking.status}</Badge></TableCell><TableCell className="text-right"><Button variant="ghost" size="sm" disabled={booking.status === 'Cancelled'} onClick={() => setCancelTarget(booking)}><XCircle className="h-4 w-4" /> Cancel</Button></TableCell></TableRow>)}</TableBody></Table><Pagination page={page} totalPages={bookings.data.totalPages} onPageChange={setPage} /></> : <EmptyState title="No bookings found" description="Change filters or wait for reservations." />}</CardContent></Card>
