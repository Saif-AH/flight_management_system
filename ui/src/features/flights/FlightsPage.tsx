...
const mapFlightFieldErrors = (errors: Record<string, string[]>) => {
  const mapped: Record<string, string[] | undefined> = {
    flightNumber: errors.flightNumber ?? errors.FlightNumber,
    originAirportId: errors.originAirportId ?? errors.departureAirportId ?? errors.DepartureAirportId,
    destinationAirportId: errors.destinationAirportId ?? errors.arrivalAirportId ?? errors.ArrivalAirportId,
    departureTimeUtc: errors.departureTimeUtc ?? errors.DepartureTimeUtc,
    arrivalTimeUtc: errors.arrivalTimeUtc ?? errors.ArrivalTimeUtc,
    status: errors.status ?? errors.Status,
    capacity: errors.capacity ?? errors.totalSeats ?? errors.TotalSeats
  };

  return Object.fromEntries(Object.entries(mapped).filter(([, value]) => value)) as Record<string, string[]>;
};
...
export function FlightsPage() {
  ...
  const params = { PageNumber: page, PageSize: 10, Search: search || undefined, Status: status || undefined, SortBy: 'departureTimeUtc', SortOrder: 'asc' as const };
  ...
  const saveMutation = useMutation({
    ...
    onError: (error) => { setServerErrors(mapFlightFieldErrors(getFieldErrors(error))); toast({ variant: 'error', title: 'Could not save flight', description: getProblemMessage(error) }); }
  });
  ...
  const saveFlight = (values: FlightFormValues) => { setServerErrors({}); saveMutation.mutate(values); };
